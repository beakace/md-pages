...
export default function OpusProcessSection() {
  ...
  return (
    <section ref={containerRef} className="py-28 md:py-36 px-6 md:px-16 lg:px-24 bg-washi dark:bg-surface-dark" id="proces">
      ...
      <div className="max-w-4xl mx-auto flex flex-col gap-24 md:gap-32 relative">
        <div className="absolute left-6 md:left-[5.5rem] top-0 bottom-0 w-px bg-graphite/8 dark:bg-chalk/8 -z-10" />

        {steps.map((step, idx) => (
          <div
            key={idx}
            className="opus-process-step flex flex-col md:flex-row gap-6 md:gap-14 items-start relative"
          >
            <div className="w-12 h-12 md:w-44 md:h-28 shrink-0 relative flex items-center justify-center bg-washi dark:bg-surface-dark organic-border border border-graphite/8 dark:border-chalk/8 shadow-[0_1px_6px_rgba(28,27,24,0.04)] dark:shadow-none z-10">
              <span className="absolute top-2 left-2.5 font-mono text-[10px] text-graphite/40 dark:text-chalk/30 opus-step-text">
                {step.num}
              </span>
              <svg viewBox="0 0 100 100" className="w-1/2 h-1/2 text-accent">
                <path
            ...
            <div className="flex-1 pt-1 relative">
              <h3 className="opus-step-text font-serif text-2xl md:text-3xl text-ink dark:text-chalk mb-3">
                {step.title}
              </h3>
              <p className="opus-step-text font-sans text-base md:text-lg text-graphite/70 dark:text-chalk/60 leading-relaxed max-w-lg">
                {step.desc}
              </p>

              <div
                className={`opus-step-note absolute -right-2 md:-right-20 top-0 font-handwritten text-lg md:text-xl text-accent/80 ${step.noteRot} hidden sm:block`}
              >
                {step.note}
                <svg
